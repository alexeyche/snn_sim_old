...
AdExLayer *createAdExLayer(const size_t N, bool saveStat) {
    ...
    l->refr = (double*) malloc( l->N * sizeof(double) );
...
void simulateAdExLayerNeuron(AdExLayer *l, const size_t *id_to_sim, const double *I, const Constants *c) {
    if(fabs(l->refr[ *id_to_sim ]) > 0.000001) {
        l->refr[ *id_to_sim ] -= c->preproc->dt;
    } else {
        double dV = *I - c->adex->gL * ( l->V[ *id_to_sim ] - c->adex->EL );
        if(c->adex->slope > 0.0) {
